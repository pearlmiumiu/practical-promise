...
function problemA () {
  ...
  // promise version
  // ???
  let result=[promisifiedReadFile('poem-two/stanza-01.txt'), promisifiedReadFile('poem-two/stanza-02.txt')];
  console.log(result);
  Promise.all(result)
  .then(function(success){
    success.forEach(blue);
    console.log('done');
    });
}
...
function problemB () {
  ...
  let result=filenames.map(promisifiedReadFile);
  //console.log(result);
  Promise.all(result)
   .then(function(success){
    success.forEach(blue);
